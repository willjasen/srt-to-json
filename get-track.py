...
def process_directory(base_dir):
    ...
    mp3_files = glob(os.path.join(usb_album_dir, '*.mp3'), recursive=True)
    data_list = []
    ...
    for filename in os.listdir(srt_files_dir):
        if filename.endswith(".srt"):
            ...
            if srt_track_name in track_numbers:
                ...
                mp3_file_path = os.path.join(usb_album_dir, f"{track_number} - {new_mp3_filename}.mp3")
                if os.path.exists(mp3_file_path):
                    os.rename(mp3_file_path, new_mp3_filename + '.mp3')
    
    print(f"Renamed: {srt_filename}")

# Get all directories under the output directory
output_dir = "/Users/willjasen/Library/Mobile Documents/com~apple~CloudDocs/wallace-thrasher/-to convert subtitles and upload-"
directories = [os.path.join(output_dir, d) for d in os.listdir(output_dir) if os.path.isdir(os.path.join(output_dir, d))]
